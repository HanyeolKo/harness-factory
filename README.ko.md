...
필요한 런타임을 따로 지정하지 않으면 `build-harness`가 세 런타임의 어댑터를 모두 생성합니다. 먼저 프로젝트를 분석한 뒤, 코드에서 안전하게 추론할 수 없는 목적, 승인 게이트, 완료 기준, 보고 방식만 짧게 확인합니다.
...
새 하네스는 이 선택을 `harness/harness-spec.json`에 기록합니다.
