...
기존 skill 수정은 기준선·파일 소유권·preservation manifest를 먼저 기록하고 delta plan의 canonical·projection만 갱신한다. 사용자 소유 문서와 지속 메모리는 승인 없이 변경하지 않는다.
...
10. `verify-harness`로 memory index와 전후 preservation manifest까지 확인하고 연결 evaluator를 실행한다. 새 parity fail은 pending event로 추가한다.
