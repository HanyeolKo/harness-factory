...
3. `state/self-evaluation.json`에서 pending event와 마지막 verdict를 확인한다.
4. `memory/INDEX.md`에서 현재 작업에 필요한 `active` 항목만 선택한다.
5. `budget/CONTEXT-BUDGET.md`의 마지막 소진 기록을 읽는다.
6. `state.json.current.refs`와 선택한 memory 항목만 추가로 읽는다.
7. 목적·다음 행동·완료 evaluator·pending event·마지막 verdict를 복원할 수 없으면 `improve.coldstart_fail = true`로 기록하고 `coldstart-fail` pending event를 중복 없이 추가한다.
8. `loops/EXECUTION-LOOP.md`를 시작한다.
...
| 메모리 | `memory/INDEX.md` | 필요한 지속 메모리만 선택 |
