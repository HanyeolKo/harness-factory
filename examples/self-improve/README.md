...
- 지속 메모리는 `harness/memory/INDEX.md`에서 필요한 항목만 선택하고, 일반 내용 변경은 결정적 검증만 수행한다.
...
포함된 memory index는 progressive disclosure와 보존 형식의 예시다. 상태·큐는 `state/state.json`, 실행 사건은 `ledger/journal.jsonl`이 정본이며 memory에 복제하지 않는다.
