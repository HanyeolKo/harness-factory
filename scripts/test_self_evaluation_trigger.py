...
class SelfEvaluationTriggerTests(unittest.TestCase):
    ...
    def test_canonical_hash_excludes_operational_and_memory_content(self) -> None:
        with WorkspaceDirectory() as target:
            harness = build_harness(target)
            ...
            excluded = {
                ...
                "memory/project-note.md": "durable note",
            ...
            unchanged, _ = run_checker(harness)
            self.assert_decision(unchanged, "none")
            ...
            self.assertTrue(policy_changed["mandatory"])
            self.assertIn("canonical-contract-change", policy_changed["reasons"])
            frozen = freeze_decision(harness, policy_changed)
            code, recorded, _ = run_recorder(
                harness, "full", "neutral", decision_file=frozen
            )
            self.assertEqual(0, code)
            self.assertTrue(recorded["recorded"])
            after_ack, _ = run_checker(harness)
            self.assert_decision(after_ack, "none")
