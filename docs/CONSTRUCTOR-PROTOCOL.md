...
- `harness/` 없음은 `create`, 유효한 runtime-neutral spec은 `improve`, 부분·레거시 구성은 `reconcile`로 분류한다.
- 기존 하네스는 원 validator/evaluator 기준선, 파일 소유권, preservation manifest를 먼저 확보하고 delta plan만 적용한다.
- delta는 `unchanged|add|modify-proposed|conflict|approval-required`로 분류하며 승인 전 삭제·이름 변경·의미 교체를 금지한다.
- schema 1.1은 `memory.index`, `preserve-and-reconcile`, line budget을 요구한다. 일반 memory 내용은 결정적 검증만 수행하고 `harness-spec.json`·`HARNESS.md`의 memory 정책·라우팅 의미 변경만 full 평가한다.
...
1. `create|improve|reconcile` 모드, 기준선, 소유권, preservation manifest와 delta plan 확정
2. memory와 self-evaluation을 모두 갖춘 schema 1.1 spec 및 모든 skill evaluator 링크 검증
3. JSON key, ID, 상대경로, 참조, DAG 검증
4. 공통 team/agent/skill/loop/recovery/state/ledger와 `memory/INDEX.md` 렌더링
5. task evaluator와 `scope:harness,type:experiment` evaluator 렌더링
6. `evaluation/suites/targeted.json`에 세 reason의 결정적 metric 렌더링
7. checker, recorder, self-evaluation state 렌더링
8. canonical contract는 별도 hash하되 일반 memory 내용은 제외하고 `watched_paths`에는 provider exact managed artifact만 등록
9. 선택 provider adapter 투영
10. validator와 CHECKLIST 수행
11. baseline full evaluation을 수행하고 recorder로 ACK하거나, 초기 mandatory event를 pending으로 명시
...
1. 현재 spec, state, ledger, provider 목록과 memory index 읽기
2. 기준선, 파일 소유권, preservation manifest 기록
3. 변경 전 `verify-harness`
4. 충돌 ID와 영향 handoff/evaluator 분석
5. spec과 공통 정본 변경; 지속 메모리 변경은 index와 원자 처리
6. 선택 provider의 exact managed artifact 재투영
7. 새 projection/wrapper/config의 exact path만 `watched_paths`에 반영
8. 해당 mandatory event를 pending events에 중복 없이 추가
9. 변경 후 `verify-harness`, memory index, preservation manifest 검사; 새 parity fail은 구조 복구
10. checker를 다시 실행하고 유효한 decision만 라우팅
...
진입에는 completed full report의 하네스 귀속 근거 또는 명시적 evidence-backed 사용자 요청이 필요합니다. 구조 오류와 미해결 parity는 먼저 결정적 verify/recovery로 복구합니다.

1. 하네스 귀속 근거와 겨냥 지표 확인
2. baseline·preservation manifest·delta plan·rollback 조건을 `maintenance/runs/<change-id>/`에 기록
3. 변경 1~2개 선택
4. 공통 spec/문서 선변경
...
- 적용 모드, delta plan, preservation manifest, memory index와 생성·수정 파일 및 보존한 state/ledger
...
10. 사용자 구성과 지속 메모리를 승인 없이 삭제·덮어쓰기·이름 변경하지 않는다.
