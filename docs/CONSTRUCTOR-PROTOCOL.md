...
- The target project owns `harness/`, state, append-only ledger, memory, evaluation runs, and reports.
...
Classify the target as `create` when no harness exists, `improve` when a valid runtime-neutral spec exists, or `reconcile` for a partial or legacy layout. Before changing an existing harness, capture its validators and evaluators, baseline, file ownership, and preservation manifest. Classify each delta as `unchanged|add|modify-proposed|conflict|approval-required`. Deletion, rename, split, merge, or semantic replacement requires explicit approval.

Schema 1.1 requires `memory` and event-driven `self_evaluation`. Existing schema 1.0 or 1.1 specs may omit `communication` and the newer reading-cost fields. Keep those specs valid, apply English presentation defaults, and add new fields only through an explicit preservation-aware delta. Do not auto-translate existing project-owned content.
...
2. Inspect target root rules, README and docs, modules, build/test/lint/CI, data contracts, existing agents, skills, hooks, evaluators, state, and memory indexes.
...
5. Ask at most two batches: core Q1–Q4, then at most four relevant conditional questions. Q4 combines cost sensitivity with user-facing report language and terminology.
6. When the user delegates choices, apply the schema 1.1 defaults and Claude, Codex, and Gemini targets, then disclose them in the delivery report.
...
- Exclude `communication.report_language` and `communication.terminology` from the harness-effect canonical hash. Artifact language and effect-bearing instructions remain hashed.
...
1. Resolve `create|improve|reconcile`; freeze baseline, ownership, preservation manifest, and classified delta plan.
...
5. Render task evaluators and the `scope: harness`, `type: experiment` evaluator.
...
7. Render the checker, recorder, and self-evaluation state.
8. Hash the effect-bearing canonical contract separately. Exclude ordinary memory content, index rows, and presentation-only communication fields. Put only exact selected-provider managed artifacts in `watched_paths`.
9. Project every selected provider adapter.
10. Run the validator, instruction and memory budgets, provider parity, linked task evaluators, cold-start, and `CHECKLIST.md`.
...
1. Read the spec, state, ledger, selected providers, report settings, and only relevant indexed memory.
...
Event mapping is agent → `agent-change`, skill → `skill-change`, evaluator → `evaluator-change`, common effect contract → `canonical-contract-change`, and provider artifact → `adapter-change`. A report-language or terminology-only change is presentation-only and does not create a harness-effect change event. Never reinitialize queues, append-only ledgers, or evaluation runs.
...
Evaluator weakening, approval-gate bypass, evidence deletion, hidden failures, unrelated state reset, and one-provider semantic drift are forbidden.
...
Run the structural, provider, evaluator, trigger-fixture, preservation, budget, and cold-start checklist. Make at most three correction rounds, and disclose every remaining failure.

The delivery report follows `communication.report_language` and `communication.terminology` while keeping technical tokens exact. Include mode, delta, preservation, files changed, retained state and ledger, defaults, topology, providers, task and harness evaluation commands, trigger/ACK settings, budgets, verification rounds, checks not run, and residual failures.
...
7. Selected providers preserve equivalent agent, skill, evaluator, gate, and handoff meaning.
8. A trigger never modifies the harness by itself.
9. The factory never centrally owns project state.
10. User configuration, durable memory, and unknown-origin files are not deleted, overwritten, translated, or renamed without approval.
11. Internal canonical prose is concise English; user-facing presentation remains separate.
12. Presentation-only communication changes do not affect the harness-effect hash.
