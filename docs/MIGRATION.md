...
## 0.3.0과 schema 1.2로 전환

0.3.0은 기존 schema 1.0·1.1의 의미를 바꾸지 않습니다. 새 하네스만 schema 1.2와 명시적 `core|adaptive|governed` 범위를 사용합니다. 기존 하네스를 1.2로 옮기는 작업은 단순 버전 치환이 아니라 전체 구조 변경입니다.

1. 기존 목적, 평가기, 상태, 원장, 메모리, 평가·학습 증거와 선택 런타임을 목록화합니다.
2. 목적을 충족하는 가장 작은 범위를 제안하고 사용자 확인을 `maintenance/runs/<change-id>/delta-plan.json`에 기록합니다.
3. `core`로 축소할 때는 메모리·평가·개선 증거의 보존, 보관, 삭제 대상을 정확히 제시하고 승인받습니다.
4. `adaptive`는 지속 메모리나 반복·장기 운영, 하네스 효과 측정이 실제로 필요할 때 선택합니다.
5. `governed`는 감사·규정 준수·개발자 이해 증거 또는 Learning Gate가 필요할 때 선택합니다.
6. 호출 중인 런타임 하나만 기본 투영하고 추가 런타임은 확인된 경우에만 생성합니다.
7. 구조 검증과 기존 작업 평가기를 통과하기 전에는 이전 파일을 제거하지 않습니다.

범위는 자동으로 바꾸지 않습니다. 이후 용도가 달라지면 새 `improve|reconcile` 작업에서 변경 이유와 보존 계획을 다시 확인합니다.
...
- `create|improve|reconcile` 모드 판정
...
- 파일 소유권, 기존 memory 후보, preservation manifest
- `maintenance/runs/<change-id>/`의 분류된 delta plan과 전후 보존 manifest
...
- `memory.index: harness/memory/INDEX.md`, `policy: preserve-and-reconcile`, line budget
...
`targeted.json`은 `cost-regression|retry-pressure|deterministic-sample`을 고정 결정적 metric에 매핑합니다. 기존 memory는 원 위치를 index에 등록하고 사용자 소유·출처 불명 파일은 승인 없이 이동·삭제하지 않습니다. 초기 state에는 `canonical-contract-change` pending event와 빈 managed hash를 둡니다. 첫 full baseline을 완료하면 recorder로 ACK합니다.
...
기존 periodic retro는 checker의 interval/cooldown/budget으로 바꾸되 LLM을 직접 호출하지 않습니다. 일반 memory 내용과 index 행은 canonical full-trigger hash에서 제외하고 `verify-harness`로 결정 검증하며, `harness-spec.json`·`HARNESS.md`의 memory 정책·라우팅 의미 변경만 canonical contract change로 취급합니다. canonical·agent·skill·evaluator·adapter 변경과 cold-start/parity 실패는 mandatory pending event입니다. cold-start false→true, parity pass→fail 전환 때 새 event를 중복 없이 추가합니다.
...
1. schema 1.1의 memory+self-evaluation parse, 모든 skill evaluator 링크, evaluator scope/type
2. memory index 7열, active 경로, 중복 ID/path, line budget, preservation manifest
3. checker와 recorder fixture; malformed decision file, decision mismatch, stale managed hash ACK 거부
4. `input-invalid:*`가 effect evaluation/LLM을 열지 않는지 확인
5. adapter/parity reason이 verify를 선행하는지 확인
6. targeted reason이 고정 deterministic suite와 정확히 매핑되는지 확인
7. full ACK가 frozen pending/failure만 ACK하고 평가 중 생긴 새 incident를 보존하는지 확인
8. watched exact managed artifact의 drift·삭제 검출과 unrelated file 제외
9. Claude·Codex·Gemini byte parity, native path, root managed block
10. cold-start false→true와 parity pass→fail incident 재검출
11. 기존 task evaluator, baseline full experiment, state/ledger 보존 확인
...
state, ledger, evaluation evidence, 사용자 memory는 제거 후보가 아닙니다.
