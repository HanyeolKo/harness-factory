...
새 하네스를 만들 때 사용자가 읽을 변경 보고서와 학습 문서를 어디에 정리할지 묻습니다. 선택값은 `harness/policies/reporting.json`에 저장합니다.

- `file`: 기본값. `harness/reports/`의 프로젝트 파일을 그대로 사용합니다.
- `notion`: 사용자가 지정한 Notion 페이지 또는 데이터베이스에 읽기용 사본을 게시합니다.
- `slack`: 사용자가 지정한 Slack 채널 또는 대화에 읽기용 사본을 게시합니다.
