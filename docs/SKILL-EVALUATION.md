...
- common contract, memory index, evaluator links, targeted suite, checker/recorder/state를 생성한다.
- 기존 하네스는 `create|improve|reconcile` mode와 preservation manifest를 기준으로 delta만 적용한다.
...
- 변경 event와 exact watched path를 원자적으로 갱신하고 memory 변경은 index와 함께 처리한다.
- 기존 evaluator 기준선과 전후 preservation manifest를 통과해야 한다.

### verify-harness

- schema, refs, DAG, access, root block, byte parity, watched drift, memory index, cold-start를 결정적으로 검사한다.
...
- 기준선·preservation manifest와 delta plan에서 1~2개 변경 후 task structural evaluator, 원 task evaluator, full experiment를 재실행한다.
...
| 일반 memory 내용/index 행 변경 | hash 불변 | `verify-harness`만 실행 |
| memory 정책·라우팅 변경 | `full`, mandatory | canonical contract 평가 |
...
8. 일반 memory 내용 변경만으로 full LLM 평가 실행
...
cold-start는 목적, next action, 연결 task evaluator, 필요한 memory index 항목, pending event, 마지막 effect verdict를 파일에서 복원해야 합니다. 과거 대화를 요구하면 fail입니다.
