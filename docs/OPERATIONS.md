...
## 기존 하네스 융화와 memory index

반복 호출은 대상 하네스를 팩토리로 흡수하거나 전면 재생성하지 않습니다. `create|improve|reconcile` 모드를 정하고 원 validator/evaluator 기준선, 파일 소유권, preservation manifest를 남긴 뒤 delta plan만 적용합니다. 기존 ID·state·ledger·evaluator·gate·사용자 규칙·출처 불명 파일은 명시 승인 없이 삭제·이름 변경·의미 교체하지 않습니다.

`harness/memory/INDEX.md`는 지속 메모리의 탐색 인덱스입니다. active 경로, ID/path 중복, 7열, line budget을 `verify-harness`가 결정적으로 검사합니다. 현재 상태와 사건은 memory에 복제하지 않고 state와 ledger에 둡니다.
...
canonical contract는 별도 hash하되 일반 `memory/` 내용과 index 행 갱신은 제외하고 결정적 memory 검증으로 처리합니다. memory 정책·라우팅의 정본은 `harness-spec.json`과 `HARNESS.md`이며 이 정본의 의미 변경은 canonical contract change입니다. `self_evaluation.watched_paths`는 선택 provider의 root guidance 파일, spec 각 skill projection, namespaced agent wrapper, 생성 config만 나열합니다. provider root 전체와 unrelated user skill/agent는 제외합니다.
...
1. baseline, 파일 소유권, preservation manifest와 직전 변경의 예상 효과를 `maintenance/runs/<change-id>/`에 고정
2. completed full report의 하네스 귀속 근거 또는 사용자의 명시적 evidence-backed 요청 확인
3. 한 번에 1~2개 변경만 delta plan으로 선택
4. spec과 공통 파일을 먼저 변경
5. delta의 선택 provider managed artifact를 재투영하고 memory 변경은 index와 원자적으로 처리
...
- `memory/INDEX.md`: 지속 메모리 경로·요약·출처·검증일·상태
...
5. `harness/memory/INDEX.md`에서 현재 unit에 필요한 항목만 선택
6. `harness/state/self-evaluation.json`
7. 현재 unit refs

목적, 현재 단계, 즉시 다음 행동, 연결 task evaluator, 선택 memory, pending harness evaluation과 마지막 verdict를 복원하지 못해 `coldstart_fail`이 false→true가 되면 pending events에 `coldstart-fail`을 중복 없이 추가합니다.
