...
4. `{{HARNESS_ROOT}}/state/state.json`에서 `phase`, `next_action`, `current`, `queue`, `improve`를 확인하고 `{{HARNESS_ROOT}}/memory/INDEX.md`에서 현재 단위에 필요한 지속 메모리만 선택한다. 새 단위 착수 전 RETRO-CHECK가 발동했거나 미적용 회고 제안서가 있으면 `{{SKILL_NAME}}-retro` 절차를 먼저 자동 실행한다.
...
7. 하네스 문서를 고치면 관련 파일 맵, `team/`, `ledger/DECISIONS.md`, `state/state.json`을 같은 변경 단위에서 갱신한다. 지속 메모리가 생성·이동·대체·보관되면 `memory/INDEX.md`도 함께 갱신한다.
