...
1. **최대 2회 배치**: 신규 생성의 1차 배치는 핵심 4문항(Q1~Q4)으로 고정. 기존 `improve|reconcile`은 현재 spec·state·결정 기록과 이번 요청에서 이미 확정된 답을 재사용하고, 충돌하거나 목적 변경 가능성이 있는 항목만 묻는다. 2차 배치는 필요한 것만 최대 4문항.
...
### Q1. 대상·목적·산출물 (신규 목적 확정 — 생략 불가)

> 이 하네스가 관리할 대상은 무엇인가요? (프로젝트 전체 / 특정 대형 작업) 그리고 주된 산출물의 형태는? (코드 / 문서 / 데이터 / 혼합)

- **질의 방식**: Phase 0 수집 자료로 세운 목적 가설을 함께 제시하고 확인·수정받는다. 예: "수집한 자료로는 ~를 위한 하네스로 보입니다. 맞나요, 아니면 다른 목적인가요?" 가설 없이 백지 질문하지 않되, 가설을 답으로 단정하지도 않는다. 단, 기존 spec의 목적을 유지한 채 개선해 달라는 요청은 그 목적의 재확인으로 간주하고 같은 질문을 반복하지 않는다. 요청과 기존 목적이 충돌할 때만 확인한다.
...
> 기존 하네스·규칙 파일이 있습니다. 현재 구성을 보존한 개선·융화 방식으로 진행할까요, 명시적 마이그레이션 범위를 따로 지정할까요?

- **기본값**: 기존 runtime-neutral spec은 `improve`, 부분·레거시 구성은 `reconcile`. 별도 `harness/` 정본과 기존 규칙을 유지하고 관리 블록만 namespace 단위로 upsert한다. 삭제·이름 변경·의미 교체는 하지 않는다.
- **매핑**: `{{HARNESS_ROOT}}`, 기존 파일 수정 여부, 적용 모드, 보존 manifest, delta plan
...
| EXISTING_HARNESS_MODE | 기존 없음=create / 유효 spec=improve / 부분·레거시=reconcile. 기존 발견 시 개선 우선 |
| MEMORY_INDEX | `<HARNESS_ROOT>/memory/INDEX.md`, preserve-and-reconcile, 생성·이동·대체·보관과 원자적 갱신 |
