...
기존 evaluator 수정은 기준선 결과와 preservation manifest를 남기고 pass condition 완화 없이 delta만 적용한다.
...
10. `pending_events`에 `evaluator-change`와 필요한 `adapter-change`를 추가하고 `verify-harness`로 memory index·preservation manifest까지 검사한다. parity pass 전 effect evaluation은 금지한다.
