...
3. spec과 공통 agent/skill/evaluator 참조, 정상 handoff DAG, approval gate, state와 journal, `memory.index`의 7열·active 경로·ID/path 중복·line budget을 검사한다.
...
6. 하네스가 변경된 경우 trigger checker와 변경을 촉발한 원 evaluator를 추가 실행한다. 일반 memory 내용·index 행 변경은 이 결정적 검증만 수행하고, `harness-spec.json`·`HARNESS.md`의 memory 정책·라우팅 계약 변경만 full 평가 신호로 취급한다.
