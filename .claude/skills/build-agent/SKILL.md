...
1. **DISCOVER** — 대상 domain, 기존 agent, handoff, evaluator, root 규칙과 preservation manifest를 확인한다. 같은 책임이 이미 있으면 중복 역할보다 기존 역할 확장을 우선하고 사용자 소유 파일·지속 메모리는 delta 밖에서 보존한다.
...
9. **VERIFY** — `verify-harness`로 memory index와 전후 preservation manifest까지 검사하고 영향받은 원 evaluator를 재실행한다. parity가 pass→fail로 바뀌면 `parity-fail`을 중복 없이 추가한다.
