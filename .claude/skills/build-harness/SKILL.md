...
- In `localized` mode, use natural report-language prose, preserve machine tokens rather than surrounding jargon, and introduce an exact technical term once only when it helps source reading.
...
6. **Install Learning Assist** — use it on request for deeper explanation or comprehension checks. It starts from the Change Report, reads only relevant actual diff/source, and uses the same plain-language-first style. Apply the configured terminology mode and run the reader wording check before publishing. A voluntary quiz while the Learning Gate is disabled is advisory and never blocks delivery.
